...
public class OrderController {
    ...
    public ObjectNode createPaymentLink(@RequestBody CreatePaymentLinkRequestBody request) {
        ...
        try {
            ...
            orderDetailService.createOrderDetail(order, request);
            ...
            final int price = request.getPrice() * quantity;
            // Gen order code
            String currentTimeString = String.valueOf(String.valueOf(new Date().getTime()));
            long orderCode = Long.parseLong(currentTimeString.substring(currentTimeString.length() - 6));

            ItemData item = ItemData.builder().name(productName).price(price).quantity(quantity).build();
